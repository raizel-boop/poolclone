...
function Canvas2D_Singleton() {
	...
	this._areaoffset = new Vector2(0,0);
}
...
Object.defineProperty(Canvas2D_Singleton.prototype, "aoffset",
    {
        get: function () {
            return this._areaoffset;
        },
		set: function (vector) {
			this._areaoffset = vector;
		}
    });
...
Canvas2D_Singleton.prototype.resize = function () {
	...
	//update new offset to calculate mouse position
	Canvas2D.aoffset = new Vector2( ((window.innerWidth - newWidth) / 2),((window.innerHeight - newHeight) / 2) );
...
Canvas2D_Singleton.prototype.drawLine = function(start, end) {
    var canvasScale = this.scale;
...
Canvas2D_Singleton.prototype.drawRect = function(position, size) {
    var canvasScale = this.scale;
	
	this._canvasContext.save();
    this._canvasContext.scale(canvasScale.x, canvasScale.y);
	this._canvasContext.beginPath();
	this._canvasContext.rect(position.x, position.y, size.x, size.y);
	this._canvasContext.stroke();
	this._canvasContext.restore();
};
